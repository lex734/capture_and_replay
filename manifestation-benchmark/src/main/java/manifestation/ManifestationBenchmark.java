...
public class ManifestationBenchmark {
    ...
    private static final String TRACE_FILE_NAME = "trace.bin";
        ...
        for (String cls : classes) {
            ...
            for (int r = 0; r < TOTAL_REPLAY_RUNS; r++) {
                ...
                ReplayResult rr = runReplay(
                    replayJarAbs, sctbenchJarAbs, cls, workDir, propsFile);
                replayRuns++;
    ...
    private static CaptureResult runCapture(
        ...
        if (!Files.exists(workDir.resolve(TRACE_FILE_NAME))) {
    ...
    // The replay agent reads trace.bin from its working directory.
    // Each buggy capture work dir already contains the captured trace.bin,
    // so we run the replay process with that dir as its working directory.
