...
This branch does not capture field/array values, so value-agreement and injection
metrics are intentionally unavailable here. The fidelity benchmark therefore
focuses on event coverage, incompleteness, divergence, and outcome reproduction.
...
Build the benchmark jar separately:

```bash
mvn -f fidelity-benchmark/pom.xml -DskipTests package
```
