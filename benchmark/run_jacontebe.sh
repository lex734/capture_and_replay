...
FALLBACK_SUBJECT_DIR="$REPO_ROOT/benchmark/fray-benchmark/bms/JaConTeBe"
INSTALL_SCRIPT="$FALLBACK_SUBJECT_DIR/scripts/install.sh"
...
Runs each selected JaConTeBe test sequentially:
  1. stages sources with the JaConTeBe install script into an isolated temp workdir
...
if [ ! -d "$FALLBACK_SUBJECT_DIR" ]; then
  echo "ERROR: fallback JaConTeBe directory not found at $FALLBACK_SUBJECT_DIR"
  exit 1
fi

if [ ! -f "$INSTALL_SCRIPT" ]; then
  echo "ERROR: JaConTeBe install script not found at $INSTALL_SCRIPT"
  exit 1
fi
...
stage_test() {
  local test_name="$1"
  local work_dir="$2"
  local staged_build_dir="$FALLBACK_SUBJECT_DIR/build/$test_name"

  rm -rf "$staged_build_dir" "$work_dir/source"
  bash "$INSTALL_SCRIPT" orig "$test_name"
  if [ ! -d "$staged_build_dir" ]; then
    echo "Missing staged build output for $test_name" >&2
    return 1
  fi

  mkdir -p "$work_dir/source"
  cp -a "$staged_build_dir"/. "$work_dir/source"/
}
...
create_workdir() {
  ...
  ln -s "$FALLBACK_SUBJECT_DIR/versions.alt" "$work_dir/versions.alt"
  printf '%s\n' "$work_dir"
}
...
prepare_special_test() {
  ...
  case "$test_name" in
    ...
    jdk7_6)
      ...
      cp "$work_dir/source/edu/illinois/jacontebe/globalevent/GlobalDriver.class" \
...
for test_name in "${tests_to_run[@]}"; do
  ...
  stage_test "$test_name" "$work_dir" >"$out_dir/install.stdout" 2>"$out_dir/install.stderr" || stage_rc=$?
